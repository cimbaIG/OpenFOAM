...
#   include "createMesh.H"
    ...
    const scalar spwn(readScalar(perturbDict.lookup("spwn")));
    const scalar stwn(readScalar(perturbDict.lookup("stwn")));
    ...
    //spanwise wavenumber - spwn
    const scalar betaPlus = 2.0*Foam::mathematicalConstant::pi*(1.0/spwn);
    const scalar sigma = 0.00055;
    //streamwise wave number - stwn
    const scalar alphaPlus = 2.0*Foam::mathematicalConstant::pi*(1.0/stwn);
