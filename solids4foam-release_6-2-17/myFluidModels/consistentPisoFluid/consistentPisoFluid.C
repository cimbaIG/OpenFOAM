...
addToRunTimeSelectionTable(fluidModel, consistentPisoFluid, dictionary);
...
tmp<vectorField> consistentPisoFluid::patchViscousForce
(
    const label patchID
) const
{
...
tmp<scalarField> consistentPisoFluid::patchPressureForce
(
    const label patchID
) const
{
...
#       include "calcPressureGradient.H"
        ...
        if (nOuterCorr == 1 && runTime().timeIndex() == 1)
        {
            nOuterCorr = 100;
        }
    }
